...
d3dreset d3dreset_org;
d3dreset d3dreset_ptr;
...
void InitD3DVSync(BOOL enabled)
{
	...
	if (d3dreset_org == NULL && !vsyncInitialized && deviceglobal)
	{
		vsyncInitialized = TRUE;

		d3dreset_org = (d3dreset)((*(uintptr_t**)deviceglobal)[14]);
		MH_CreateHook(d3dreset_org, &d3dreset_my, reinterpret_cast<void**>(&d3dreset_ptr));
		MH_EnableHook(d3dreset_org);
		if (SetInfoObjDebugVal)
		{
