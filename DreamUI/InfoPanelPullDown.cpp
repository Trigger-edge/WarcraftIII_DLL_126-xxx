...
namespace InfoPanel {
	...
	static std::map<PullDown*, uint32_t> PullDownActiveIndexMap;
	...
	static void PullDownHandler(Observer* ob, uint32_t evtId) {
	...
	static void PullDownItemHandler(Observer* ob, uint32_t evtId) {
		PullDownItemHandlerParam *pParam = ob->data<PullDownItemHandlerParam>();
		PullDown *pd = pParam->pd;
		uint32_t index = pParam->index;
		switch (evtId) {
		case EVENT_CLICK:
			//pd->setIndex(index);
			break;
		...
		: currentItemIndex(0), pullDownFrameIsHidden(true)
	{
		...
		for (uint32_t i = 0; i < inItemCount; ++i) {
			param.pd = this;
			param.index = i;
	...
	void PullDown::setRelativePosition(uint32_t originPos, UILayoutFrame* target, uint32_t toPos, float relativeX, float relativeY) {
		this->currentItemFrame->setRelativePosition(originPos, target, toPos, relativeX, relativeY);
	}

	void PullDown::setAbsolutePosition(uint32_t originPos, float absoluteX, float absoluteY)	{
	...
	void PullDown::setIndex(uint32_t index)	{
		expand(false);
	...
	void PullDown::toggleIndex(uint32_t index)	{
		if (!this) return;
		if (getIndex() != index)
		{
			setIndex(index);
	...
	void PullDown::setText(uint32_t index, char *format, ...)	{
		va_list args;
	...
	void PullDown::setColor (uint32_t index, float r, float g, float b, float a)
	{
	...
	std::uint32_t PullDown::getIndex() const
	{
