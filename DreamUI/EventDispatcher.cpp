...
uint32_t DispatchedLevel = 0;
const uint32_t MAX_DISPATCH_RECURSIVE_LEVEL = 15;
bool EventDispatcher::dispatch(const Event* evt) {
	...
	for (uint32_t i = 0; i < this->subscriber_count_; ++i) {
...
void EventDispatcher::listen(uint32_t event_id, EventListenerType callback) {
...
bool EventDispatcher::hasListener(uint32_t event_id) {
